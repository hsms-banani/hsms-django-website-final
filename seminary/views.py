...
def home(request):
    ...
    context = {
        ...
        'featured_news': News.objects.filter(is_published=True, is_featured=True).order_by('-created_at')[:10],
        'upcoming_events': Event.objects.filter(
            is_published=True, 
            start_date__gte=timezone.now()
        ).order_by('start_date')[:6],
        'recent_publications': Publication.objects.filter(is_published=True).order_by('-publication_date')[:6],
        'faculty_highlights': Faculty.objects.filter(is_active=True).order_by('order')[:25],
